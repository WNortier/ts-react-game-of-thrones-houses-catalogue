...
function App() {
  const [bg, setBg] = useState('/game-of-thrones-houses-catalogue/mainbg01.jpeg');
  ...
  useEffect(() => {
    ...
    const base = process.env.DEV ? '/game-of-thrones-houses-catalogue/' : '/game-of-thrones-houses-catalogue/'

    if (localStorage.getItem("disableVary") === "true") {
      setBg(base + "houses.jpeg");
    } else {
      const path = window.location.pathname
        .split("/")[process.env.DEV ? 1 : 1]?.substring(0, 5);
